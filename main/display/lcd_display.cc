...
void LcdDisplay::SetPreviewImage(std::unique_ptr<LvglImage> image) {
    DisplayLockGuard lock(this);
    if (!lock.locked() || content_ == nullptr) {
        return;
    }
...
void LcdDisplay::SetPreviewImage(std::unique_ptr<LvglImage> image) {
    DisplayLockGuard lock(this);
    if (!lock.locked() || preview_image_ == nullptr) {
...
void LcdDisplay::ScrollChat(int offset_y) {
    DisplayLockGuard lock(this);
    if (!lock.locked()) {
        return;
    }
    lv_obj_t* target = content_ != nullptr ? content_ : bottom_bar_;
    if (target == nullptr) {
        return;
    }
    lv_obj_scroll_to_y(target, lv_obj_get_scroll_y(target) + offset_y, LV_ANIM_ON);
}

void LcdDisplay::ScrollChatToBottom() {
    DisplayLockGuard lock(this);
    if (!lock.locked() || content_ == nullptr) {
        return;
    }
    ...
    lv_obj_t* last_child = lv_obj_get_child(content_, child_count - 1);
    if (last_child != nullptr && lv_obj_is_valid(last_child)) {
        lv_obj_scroll_to_view_recursive(last_child, LV_ANIM_ON);
    }
}
...
void LcdDisplay::SetEmotion(const char* emotion) {
    ...
    // EXCEPT when we are listening or speaking
    auto& app = Application::GetInstance();
    bool is_listening_or_speaking = (app.GetDeviceState() == kDeviceStateListening || app.GetDeviceState() == kDeviceStateSpeaking);
    uint32_t child_count = lv_obj_get_child_cnt(content_);
    if (strcmp(emotion, "neutral") == 0 && child_count > 0 && !is_listening_or_speaking) {
