...
bool EspVideo::SaveJpegToFile(const std::string& path, int quality) {
    if (frame_.data == nullptr || frame_.len == 0) {
        ESP_LOGE(TAG, "No captured frame available");
        return false;
    }
    ...
    bool ok = image_to_jpeg_cb(
        frame_.data, frame_.len, frame_.width, frame_.height, frame_.format, quality,
        [](void* arg, size_t, const void* data, size_t len) -> size_t {
            auto ctx = static_cast<FileEncodeContext*>(arg);
    ...
    fclose(file);
    if (!ok || ctx.failed || ctx.written == 0) {
        remove(path.c_str());
        ESP_LOGE(TAG, "Failed to save JPEG to %s", path.c_str());
        return false;
    }

    ESP_LOGI(TAG, "Saved JPEG to %s (%u bytes)", path.c_str(), static_cast<unsigned>(ctx.written));
    return true;
}
...
std::string EspVideo::Explain(const std::string& question) {
    ...
    if (!http->Open("POST", explain_url_)) {
        ...
        // Drain the queue while the encoder is running. Joining first can
        // deadlock when the queue fills after a network connection failure.
        JpegChunk chunk;
        while (xQueueReceive(jpeg_queue, &chunk, pdMS_TO_TICKS(15000)) == pdPASS) {
            if (chunk.data != nullptr) {
    ...
    bool upload_failed = false;
    {
        ...
        if (http->Write(question_field.c_str(), question_field.size()) < 0) {
            ESP_LOGE(TAG, "Failed to send photo question");
            upload_failed = true;
        }
    }
    {
        ...
        if (http->Write(file_header.c_str(), file_header.size()) < 0) {
            ESP_LOGE(TAG, "Failed to send photo header");
            upload_failed = true;
        }
    }
    ...
    while (true) {
        JpegChunk chunk;
        if (xQueueReceive(jpeg_queue, &chunk, pdMS_TO_TICKS(15000)) != pdPASS) {
    ...
    if (upload_failed) {
        http->Close();
        throw std::runtime_error("Failed to send photo");
    }

    {
        ...
        if (http->Write(multipart_footer.c_str(), multipart_footer.size()) < 0) {
            http->Close();
            throw std::runtime_error("Failed to send photo footer");
        }
    }
    // 结束块
    if (http->Write("", 0) < 0) {
        http->Close();
        throw std::runtime_error("Failed to finish photo upload");
    }
    ...
    http->Close();
    if (result.empty()) {
        throw std::runtime_error("Photo service returned an empty response");
    }
